...
if __name__ == "__main__":
    ...
    # Set tracking URI
    mlflow.set_tracking_uri(uri="http://127.0.0.1:5000")  # 또는 "./mlruns"
    ...
    with mlflow.start_run() as run:
        ...
        # Create a Conda environment for the new MLflow Model
        conda_env = {
            "channels": ["defaults"],
            "dependencies": [
                "python=3.10",  # 올바른 문자열 포맷
                "pip",
                {
                    "pip": [
        ...
        mlflow.pyfunc.log_model(
            ...
            artifacts=artifacts,
            code_path=[__file__],  # 현재 실행 중인 파일
            conda_env=conda_env
        )
        
        # Get current run_id dynamically
        current_run_id = run.info.run_id
        
        # Load and test the model
        model_uri = f"runs:/{current_run_id}/sklearn_mlflow_pyfunc"
        ld = mlflow.pyfunc.load_model(model_uri=model_uri)
